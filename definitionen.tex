...
\end{Sinput}
\begin{Soutput}
 [1]  2  2  3  3  5  6  9 10 10 11 13 14 14 15 16 17 18 18 19 20
\end{Soutput}
\begin{Sinput}
> quantile(x, prob=0.2)
\end{Sinput}
...
\[ \int f(x) := F(x) \qquad \Leftrightarrow \qquad
   f(x) = \frac{\delta}{\delta x} F(x)  \]

\noindent
Dies impliziert, dass auch die folgenden Gesetzte gelten:
\begin{itemize}
	\item Die Dichte zu einem Punkt ist immer grösser oder gleich Null.
	\[ f(x) \geq 0 \]
...
\subsubsection{Erwartungswert}
\[ E(X) = \int_{-\infty}^{+\infty} x \cdot f(x) \delta x  \]

\subsubsection{Varianz}
\[ Var(X) = E(X^2) - E(X)^2 \qquad \qquad \text{weil} \qquad 
   Var(X) = \int_{-\infty}^{\infty} (x-E(X))^2 f(x) \delta x \] 

\subsubsection{Standardabweichung}
\[ \sigma_x = \sqrt{Var(X)} \]

\subsubsection{Qunatile}
\[ q(\alpha), \qquad \alpha=\{ 0 < \alpha < 1 \} \]
\[ P(X \leq q(\alpha)) = \alpha \]
\[ F(q(\alpha)) = \alpha \qquad 
   \Leftrightarrow \qquad F^{-1}(\alpha) = q(\alpha)\]

% HIER KOMMT DIE GRAFIK REIN FÜR DIE QUANTILE DER STETIGEN VERTEILUNG
