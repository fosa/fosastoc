...
\end{Sinput}
\begin{Soutput}
 [1]  2  2  4  4  5  6  7  9 10 11 12 12 13 13 14 15 15 16 19 20
\end{Soutput}
\begin{Sinput}
> quantile(x, prob=0.2)
\end{Sinput}
...
\begin{itemize}
	\item $ f(x) \geq 0 $
	\item $ \int_{a}^{b} f(x) \delta x = F(b) - F(a) $
	\item $ \int_{-\infty}^{+\infty} f(x) \delta x = 1 $
\end{itemize}
...
\verb!t.test(x=man,y=fem,alternative='less',paired=FALSE,var.equal=TRUE)!
