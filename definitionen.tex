...
\[ \begin{array}{@{}ll}
  \text{für }x \geq 0: & f(x) = \lambda \cdot e^{-\lambda \cdot x} \\
  \text{für }x < 0:    & f(x) = 0
\end{array} \]
\begin{Schunk}
\begin{Sinput}
...
\subsubsection{Kumulative Verteilungsfunktion}
\[ \begin{array}{@{}ll}
  \text{für }x \geq 0: & f(x) = 1 - e^{-\lambda \cdot x} \\
  \text{für }x < 0:    & f(x) = 0
\end{array} \]
\begin{Schunk}
\begin{Sinput}
...
\begin{table}[h!]
	...
	t-Wert für $\beta_0$ & t value (intercept) & t-Wert (t-Test auf $\beta_0$, d.h. $ \displaystyle t = \frac{\beta_0}{\sigma}$) \\
	t-Wert für $\beta_1$ & t value & t-Wert (analog zum t für $\beta_0$) \\
