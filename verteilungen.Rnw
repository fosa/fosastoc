...
<<pois1,fig=FALSE, echo=FALSE>>=
a <- dpois(x=c(1:100), lambda=20)
b <- dpois(x=c(1:100), lambda=50)
c <- dpois(x=c(1:100), lambda=80)
plot(a, type='h', cex.lab=1.5, cex.axis=1.5, cex.main=1.5, 
     cex.sub=1.5, ylab='P(x)', xlab='x', ylim=c(0.00, 0.1),
     main='dpois()')
lines(b, type='l')
lines(c, type='p')
text(20, 0.095, "lambda=20", cex=1.5)
text(50, 0.06, "lambda=40", cex=1.5)
text(80, 0.05, "lambda=50", cex=1.5)
@
...
Ein Team hat ein sog. \emph{Embedded System} entwickelt, welches
...
Studenten hin um alles zu überprüfen und allenfalls ein Reboot
für spezielle Updates vorzunehmen.

Das Entwicklerteam erfährt am Montag von einer neu entdeckten 
Sicherheitslücke in einer der Applikationen des Systems. 
Sie müssen nun entscheiden, ob sie noch vor dem Wochenende 
eine Gruppe von Studenten für einen Reboot zum System senden 
sollen oder einfach abwarten.

Diese Fragestellung lässt sich einfach beantworten, wenn man
die zu erwatenden Attacken pro Zeit kennt (von Quellen oder
Schätzungen). Die Entwickler erfahren vom Hersteller der
Software, dass ca. alle 12 Stunden mit einem Angriff zu rechnen 
ist, aber die ersten 15 Attacken jeweils erfolglos sind.

Die Entwickler kennen mit diesen Informationen alles was sie
brauchen um die Frage korrekt aufzustellen: "`\emph{Wie
wahrscheinlich ist es, dass innert 7 Tagen mindestens 15 Attacken
ausgeführt werden?}"'

<<>>=
hpd <- 2 # hits per day
hpw <- hpd*7 # hits per week
# probability of 15 hits in one week
dpois(x=15, lambda=hpw)
# probability of at least 15 hits in one week
1-ppois(q=15, lambda=hpw)
@

<<interrupt1, fig=FALSE, echo=FALSE>>=
hpd <- 2 # hits per day
hpw <- hpd*7 # hits per week
X <- dpois(x=c(1:30), lambda=(7*2))
plot(X, cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5, 
     ylab='P(X=x)', xlab='Anzahl Attacken', main='dpois()')
@

<<interrupt2, fig=FALSE, echo=FALSE>>=
hpd <- 2 # hits per day
hpw <- hpd*7 # hits per week
X <- 1-ppois(q=c(1:30), lambda=hpw)
plot(X, cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5, 
     ylab='P(X<=x)', xlab='Anzahl Attacken', main='ppois()')
abline(h=(1-ppois(q=15, lambda=hpw)))
@

Die Analyse mit \lstinline{R} hat ergeben, dass die 
Wahrschienlichkeit genau 15 Attacken zu erhalten bei ca. $\frac{1}{10}$
liegt und die Wahrscheinlichkeit von mindestens 15 Attacken
bei ca. $\frac{1}{3}$. Die kompletten Daten sind in den Plots
zu sehen aus der Abbildung \ref{fig:embedded}

Die Entwickler können sich also zurücklehnen mit dem Argument:
"`Es ist wahrscheinlicher, dass weniger als 15 Attacken auf das 
...
<<interrupt2>>
@
\caption{Kummulative Wahrsch.}
\end{subfigure}
\caption{Wahrscheinlichkeiten von Attacken}
\label{fig:embedded}
\end{figure}
...
<<lintrans1, fig=FALSE, echo=FALSE>>=
X <- runif(n=100, min=1, max=2)
plot(X, cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5, 
     ylim=c(0, 12), ylab='', xlab='', main='X')
abline(h=mean(X))
@

<<lintrans2, fig=FALSE, echo=FALSE>>=
# X <- rbinom(n=100, size=100, prob=0.2)
Y <- (3*X+2)
plot(Y, cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5, 
     ylim=c(0, 12), ylab='', xlab='', main='Y')
abline(h=mean(Y))
@
